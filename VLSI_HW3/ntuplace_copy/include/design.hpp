...
struct Segment {
    int rowId;        // 該 segment 屬於哪一個 row
    int x1, x2;       // 可放置範圍 [x1, x2)
    std::vector<int> cellIds; // 該 segment 內的 movable cell ID
};

// ⚠️ 保留舊版 API 聲明（實際 inline 包裝在底部）
void buildSegments();
    ...
    void buildSegments() {
        ...
        for (int rid = 0; rid < (int)layout.rows.size(); ++rid) {
            ...
            for (auto& seg : segments) {
                ...
                std::sort(seg.cellIds.begin(), seg.cellIds.end(),
                          [&](int a, int b){ return insts[a].x < insts[b].x; });
            }
        }
...
// === Inline wrapper for backward compatibility ===
inline void buildSegments(Design& d) {
    d.buildSegments();
}