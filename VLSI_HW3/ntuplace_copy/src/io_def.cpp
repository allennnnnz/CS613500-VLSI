...
// capture design name from DEF header
static int designCb(defrCallbackType_e, const char* name, defiUserData){
    if (name && D) D->designName = name;
    return 0;
}
...
static int pinCb(defrCallbackType_e, defiPin* p, defiUserData){
    std::string name = p->pinName();
    int x = 0, y = 0;
    if (p->hasPlacement() || p->isPlaced() || p->isFixed()) {
        x = p->placementX();
        y = p->placementY();
    }
...
static int netCb(defrCallbackType_e, defiNet* n, defiUserData){
    ...
    for (int i=0;i<n->numConnections();++i){
        ...
        if (std::strcmp(inst, "PIN")==0){
            ...
            auto it = D->ioPinLoc.find(R.ioName);
            if (it != D->ioPinLoc.end()) R.ioLoc = it->second;
            N.pins.push_back(R);
        } else {
...
// SPECIALNETS callback: mark as special and build pins similarly
static int snetCb(defrCallbackType_e, defiNet* n, defiUserData){
...
// 在 loadDEF() 最後加入
static void validateIOPins(const Design& d) {
    int missing = 0;
    for (const auto& net : d.nets) {
        for (const auto& p : net.pins) {
            if (!p.isIO) continue;
            if (!d.ioPinLoc.count(p.ioName)) {
                ++missing;
                std::cerr << "[WARN][IO] Missing placement for IO pin: "
                          << p.ioName << " (default 0,0)\n";
            }
        }
    }
    if (missing == 0)
        std::cout << "✅ IO pins all have placement.\n";
}
...
bool writeDEF(const std::string& out, const Design& d){
    ...
    // Prefer original DEF design name if available; else infer from output filename
    std::string dname = d.designName;
    if (dname.empty()){
        dname = out;
        size_t slash = dname.find_last_of("/\\");
        if (slash != std::string::npos) dname = dname.substr(slash+1);
        size_t dot = dname.find_last_of('.');
        if (dot != std::string::npos) dname = dname.substr(0, dot);
        if (dname.empty()) dname = "design";
    }
    fprintf(fo, "DESIGN %s ;\n", dname.c_str());
    ...
    while (std::getline(fin, line)){
        ...
        if (!inComponents){
            ...
            if (line.find("END COMPONENTS") != std::string::npos) {
                ...
                if (!curInst.empty()) {
                    // 支援 "+ PLACED" 或 "+ UNPLACED" 的 component
                    bool isPlacedLine   = (line.find("+ PLACED")   != std::string::npos);
                    bool isUnplacedLine = (line.find("+ UNPLACED") != std::string::npos);

                    if (isPlacedLine || isUnplacedLine) {
                        auto it = d.instName2Id.find(curInst);
                        if (it != d.instName2Id.end()) {
                            const Inst& inst = d.insts[it->second];

                            // 找距離最近的 row 來決定朝向 (N/FS)
                            const Row* bestRow = nullptr;
                            int bestDy = std::numeric_limits<int>::max();
                            for (const auto& r : d.layout.rows) {
                                int dy = std::abs(inst.y - r.originY);
                                if (dy < bestDy) { bestDy = dy; bestRow = &r; }
                            }
                            const char* rowOri = (bestRow && bestRow->flip) ? "FS" : "N";

                            // 若是 UNPLACED，要改成 PLACED
                            if (isUnplacedLine) {
                                size_t pos = line.find("+ UNPLACED");
                                if (pos != std::string::npos)
                                    line.replace(pos, 11, "+ PLACED");
                            }
