...
void readDEF(const string &path, db::Database &db) {
    ...
    for (auto &cell : db.cells) {
        ...
        if (it != db.macros.end()) {
            ...
            // Fallback: if macro not found in LEF, assume single-site cell size
            int defW = (int)round(db.coreSite.width * db.lefDBMicron);
            int defH = (int)round(db.coreSite.height * db.lefDBMicron);
            if (defW <= 0) defW = 1;
            if (defH <= 0) defH = db.rows.empty() ? 1 : db.rows.front().stepY;
            cell.width = defW;
            cell.height = defH;
        }
    }
